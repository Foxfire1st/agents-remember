# Job — Adversarial Reviewer

> **Role + lens in one file.** This is the portable job file the `l-02-agent-orchestration` frame
> houses at the **adversarial reviewer** seat. It carries both axes: the **role** (review the
> accumulated change set at a seam) and the **lens** (refute-or-confirm across three review lenses). No
> harness overlay ships yet; author `jobs/adversarial-reviewer.<harness>.md` when one is needed.
>
