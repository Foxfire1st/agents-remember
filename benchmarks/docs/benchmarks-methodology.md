...
The source package does not commit case workspaces. It commits manifests, prompts, author results, and workspace templates. `prepare` generates the case workspace as resettable state with a source-only environment and a memory-enabled environment. The source-only environment contains only the pinned source checkout under `repos/`. The memory-enabled environment renders the workspace `AGENTS.md` from the template, clones the same pinned source checkout under `repos/`, and clones the pinned memory repository under the benchmark-local `ar-coordination/` root. Reinstalling the benchmark package can prune and refresh generated workspaces when the pinned commit, prompt, memory, template, or author results change. User-generated outputs live separately under `benchmarks/user-runs/`.
