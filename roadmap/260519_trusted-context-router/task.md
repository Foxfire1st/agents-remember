...
| Deferred provider lifecycle     | Before CGC becomes a configured relationship provider, does the installed package create any hidden repo-local artifacts beyond optional `.cgcignore`, and does it honor `KUZUDB_PATH`/`FALKORDB_PATH` exactly as main-branch source suggests?           | Deferred until CGC installation/evaluation.                                                                   |
| Deferred provider lifecycle     | Should Graphify's hard-coded secondary paths be revisited?                                                                                                                                                                                               | Only if CGC fails evaluation or Graphify becomes necessary again.                                             |
