...
2026-05-22 update: Later phase decisions supersede the blanket wording below that says v1 has no destructive operations or provider refresh/start/stop. With `ar-coordination-mcp-plus-refactor-workbench`, mutation-capable provider/worktree operations can be wired earlier as long as they are explicit typed operations over pinned facades and first tested against the isolated workbench. The still-valid safety boundary is no arbitrary shell execution, no caller-provided executables or roots, and no hidden mutations inside read-only contracts.
...
Worktree creation is important enough to support, and it should be treated as a typed operation over C-09-style contracts rather than a loose shell command. The workbench means this does not need a blanket version delay; it needs pinned facades, explicit arguments, and isolated verification before the real coordinator is touched.

Possible wiring order:

```text
1. worktree.status and worktree.plan_create
2. worktree.create once the facade contract is pinned and workbench-tested
3. worktree.cleanup/delete once the same typed contract and workbench tests exist
```
...
No unbounded output.
No hidden mutations inside read-only contracts.
Mutation-capable tools must be explicit, typed, facade-backed, and workbench-tested before real coordinator use.
```
