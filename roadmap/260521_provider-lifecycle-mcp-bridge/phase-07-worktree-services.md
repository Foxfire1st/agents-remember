...
Worktree operations are workflow safety boundaries, not convenience helpers. Start with status and planning for clarity, but the workbench means creation/cleanup wiring can be tested earlier once C-09 contracts are pinned and exposed through typed facades.

### Assumptions

- C-09 remains the governing workflow contract until replacement services are proven.
- MCP worktree mutation can be tested safely against isolated coordinator/worktree state in `ar-coordination-mcp-plus-refactor-workbench`.

### Boundaries

- Do not hide branch-pairing risks behind implicit behavior.
- No worktree mutation tool may accept caller-provided shell commands, arbitrary roots, or bypass C-09 approval gates.
...
- Use the workbench as the first target for creation/cleanup behavior.
...
| 2026-05-22T12:05 | Worktree mutation is not categorically excluded from MCP once workbench testing is available. | The risk shifts from mutation itself to preserving C-09 contracts, branch-pairing visibility, and typed operation boundaries. |

---

## Open Questions

- Which C-09-backed worktree mutation should be exposed first after status/planning contracts are pinned?
