...
> **Overlay, not a replacement.** This file overlays the portable `jobs/orchestrator.md` when the
...
> the orchestrator's duties, the spirit test, or the topology — read `jobs/orchestrator.md` for those.
>
> Resolution: `jobs/orchestrator.md` (base) → **this overlay** → settings.json orchestration block.
