...
def closeout_summary(contract: WorktreeContract) -> str:
    if contract.kind == "series":
        return (
            ...
            "exact named atomic code and memory refs and "
            "records those already-landed commits."
        )
    return (
        "Closeout preview only; no commits were created. The transaction records the accepted "
        "code commit, refreshes and commits external memory when configured, refreshes its "
        "derived ledger cache, then updates the contract."
    )
