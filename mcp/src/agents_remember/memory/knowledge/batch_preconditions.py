...
from agents_remember.memory.knowledge import anchors, families, lineage, memberships, realizations
...
def require_command_target(
    ...
    if isinstance(command, AddInvariant | SetInvariantLabel):
        _require_identity(store, index, command, pending, table="invariant")
        return
    if isinstance(command, AddFamily | SetFamilyLabel):
        _require_identity(store, index, command, pending, table="family")
        return
    ...
    if isinstance(command, AddFamilyRevision):
        ...
        return
    if isinstance(command, AddSourceAnchor | RemoveSourceAnchor):
        _require_anchor(store, index, command)
        return
    if isinstance(command, AddFamilyMember | RemoveFamilyMember):
        _require_membership(store, index, command, pending)
        return
    _require_claim(store, index, command, pending)
