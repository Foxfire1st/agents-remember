...
def _collect_stale_flags(contract: LayersContract) -> list[tuple[str, str]]:
    ...
    for name in contract.order:
        ...
        if not arrives_in:
            continue
        if _leaf_landed(str(arrives_in)):
            stale.append((name, str(arrives_in)))
    return stale


def build_report(source_root: Path, contract: LayersContract) -> LayeringReport:
    ...
    return LayeringReport(
        ...
        stale_present_flags=_collect_stale_flags(contract),
...
def check_layering(project_root: Path) -> LayeringReport:
    ...
    return build_report(source_root, contract)
