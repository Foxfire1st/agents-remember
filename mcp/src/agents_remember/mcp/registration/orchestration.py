...
def register_orchestration_tools(server: FastMCP, config: McpRuntimeConfig) -> None:
    ...
    def operator_inbox_post(
        ...
        return operator_inbox_post_payload(
            ...
            address=InboxAddress(
                lifecycle_id=lifecycle_id, agent_id=agent_id, recipient_role=recipient_role
            ),
            ...
            delivery=HostedDelivery(enabled=deliver_to_hosted),
        )
    ...
    def orchestration_nudge_manager(
        ...
        return orchestration_nudge_manager_payload(
            config,
            reason=reason,
            target=NudgeTarget(agent_id=manager_agent_id, lifecycle_id=manager_lifecycle_id),
            subject=NudgeSubject(
                subject=subject,
                agent_id=subject_agent_id,
                lifecycle_id=subject_lifecycle_id,
                artifact_path=artifact_path,
            ),
