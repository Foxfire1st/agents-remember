...
L-01's `decide` build-mode step escalates a single task to a series once its size is apparent — the
...
- **One task = one workflow = one worktree, with many commits and one integrate.** Each sub-task slice
  is committed via its own C-09 closeout (a commit) behind an explicit commit gate; the worktree stays
...
- Sub-tasks commit **incrementally** (one C-09 closeout per slice, behind a commit gate); the worktree
