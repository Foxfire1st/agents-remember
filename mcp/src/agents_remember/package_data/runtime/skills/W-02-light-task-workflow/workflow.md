...
2. apply C-02's clean-source versus dirty-source drift classification before planning against pre-existing onboarding
...
6. before any C-09 worktree starts, commit refreshed external-memory onboarding and the ledger so the worktree starts from a clean, mapped memory baseline
