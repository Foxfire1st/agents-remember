...
2. **Refresh the matching onboarding in the same editing pass**, per completed plan-section — never
   ...
   - For **new** source files, run `check_missing_onboarding` before the commit and create the
     reported missing sidecars through `C-05`; the post-code-commit memory refresh stamps them with
     the real code commit hash and date.
3. Run the checks from the C-08 resolved `system/tools.md` (lint, typecheck, complexity, tests) and
...
1. Run the C-09 closeout **preview** for the worktree (`worktree_closeout_preview`) — or
...
2. **Commit gate:** stop for explicit developer commit approval before any real commit or closeout
   apply. If required onboarding is missing, run `C-05` for the affected file and re-run the preview.
3. On approval, C-09 owns the external-memory invariant in order: commit code → refresh affected
