...
2 Frame    tasks-doctrine: reframe, pull evidence via C-04, expose truth gaps + run the job opening move
3 Decide   changes code? no -> read-only exit · yes -> ALWAYS a C-09 worktree; then pick the build mode
4 Build    implement in the worktree; refresh onboarding per completed plan-section; checks green per commit
5 Close    worktree closeout -> land per system/git-workflow.md -> cleanup -> C-11 carryover (commit-gated)
```
...
read-only answer). It does not replace the memory/core skills it calls (`C-02`, `C-04`, `C-05`,
`C-08`, `C-09`, `C-11`); it sequences them.
...
1. Every session enters L-01; the job type is a lens, re-pickable, never a gate.
...
9. Checks from the C-08 resolved `system/tools.md` run green before **each** incremental commit; the
...
11. L-01 covers everything the retired chat workflow did — the C-02 task-start gate, paired
    source+onboarding reads, the approval gate, and C-09 closeout — plus the job lens and the
