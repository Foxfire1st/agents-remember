...
- **Retrieval lean** — which `C-04` strategy leads (others are still available).
- **Decide default** — where this job usually lands at `decide` (still a real decision, not automatic).

| Job        | Opening move (in `frame`)                       | Retrieval lean (`C-04`)            | `decide` default            |
| ---------- | ----------------------------------------------- | ---------------------------------- | --------------------------- |
...
contracts the feature must respect. Defaults to a build; size decides chat vs durable W-02 task at
`decide`.
