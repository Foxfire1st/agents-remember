...
manages, so it may retire only the worker/reviewer seats of its OWN master -- it can never unseat
...
MANAGER_RETIRE_ROLES = frozenset({"worker", "reviewer"})
...
class SeatRef:
    """The retire-policy-relevant facts about one catalog seat."""
...
def check_retire_authority(actor: SeatRef, target: SeatRef) -> None:
    ...
    overrides it. A manager may then retire only a worker/reviewer of its own master; anything
    ...
    if actor.role == "manager":
        if target.role not in MANAGER_RETIRE_ROLES or target.master != actor.master:
            raise RetirePolicyError(
                "manager may retire only worker/reviewer seats of its own master "
                f"({actor.master!r}); target is {target.role!r} of {target.master!r}"
            )
        return
    if actor.role == "orchestrator":
        return
    raise RetirePolicyError(f"role {actor.role!r} has no retire authority")
