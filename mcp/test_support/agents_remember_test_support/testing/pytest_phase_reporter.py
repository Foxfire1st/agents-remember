...
class _PhaseState:
    ...
    xdist_collection_consistent: bool = True
    selected_node_ids: tuple[str, ...] = ()
    deselected_node_ids: set[str] = field(default_factory=set)
...
def pytest_sessionstart(session: pytest.Session) -> None:
    ...
    _STATE.xdist_collection_consistent = True
    _STATE.selected_node_ids = ()
    _STATE.deselected_node_ids.clear()
    _STATE.node_outcomes.clear()


def pytest_collection_finish(session: pytest.Session) -> None:
    _STATE.selected_node_ids = tuple(item.nodeid for item in session.items)
    _record_collection_finished()


def pytest_deselected(items: Sequence[pytest.Item]) -> None:
    _STATE.deselected_node_ids.update(item.nodeid for item in items)
...
def pytest_xdist_node_collection_finished(node: object, ids: Sequence[str]) -> None:
    ...
    if _STATE.selected_node_ids and collection != _STATE.selected_node_ids:
        _STATE.xdist_collection_consistent = False
    elif not _STATE.selected_node_ids:
        _STATE.selected_node_ids = collection
...
def _population_payload() -> dict[str, object]:
    selected = _STATE.selected_node_ids
    deselected = tuple(sorted(_STATE.deselected_node_ids))
    xdist = _STATE.xdist_expected_workers > 0
    return {
        "collected": len(selected) + len(deselected) if not xdist else len(selected),
        "selected": len(selected),
        "deselected": len(deselected) if not xdist else None,
        "reported": len(_STATE.node_outcomes),
        "selectedNodeIdsSha256": _node_ids_digest(selected),
        "deselectedNodeIdsSha256": _node_ids_digest(deselected) if not xdist else None,
        "xdistWorkers": _STATE.xdist_expected_workers,
        "xdistCollectionConsistent": _STATE.xdist_collection_consistent,
    }


def _node_ids_digest(values: Sequence[str]) -> str:
