...
def candidate_test_process(candidate_root: Path) -> CandidateTestProcess:
    ...
    test_support_root = root / "mcp" / "test_support"
    if not (source_root / "agents_remember" / "__init__.py").is_file():
        raise BootstrapConfigurationError(
            f"candidate does not contain mcp/src/agents_remember: {root}"
        )
    if not (test_support_root / "agents_remember_test_support" / "__init__.py").is_file():
        raise BootstrapConfigurationError(
            f"candidate does not contain mcp/test_support/agents_remember_test_support: {root}"
        )
    return CandidateTestProcess(
        candidate_root=root,
...
def hermetic_pytest_environment(
    ...
    *,
    ...
    result["PYTHONPATH"] = os.pathsep.join(
        (process.test_support_root.as_posix(), process.source_root.as_posix())
    )
