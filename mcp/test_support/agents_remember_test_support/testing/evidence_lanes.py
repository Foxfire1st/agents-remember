...
EVIDENCE_LANES = (
    ...
    EvidenceLane(
        ...
        None,
        "non-accepting explicit Dagger investigation route",
        EvidenceFidelity.CONTROLLED_INVESTIGATION,
...
def expression_for(trigger: EvidenceTrigger) -> str | None:
    ...
    if trigger is EvidenceTrigger.DIAGNOSTIC:
        raise ValueError("diagnostic evidence requires an explicit route, not a marker expression")
    return None


def category_for_item(item: pytest.Item, manifest: LaneManifest) -> EvidenceCategory:
    """Resolve exactly one declared category and refuse marker disagreement."""
    ...
    try:
        category = manifest.category_for_node(item.nodeid)
    except LaneManifestError as error:
        raise pytest.UsageError(str(error)) from error
    if provider_gates and category is not EvidenceCategory.PROVIDER_CONFORMANCE:
        raise pytest.UsageError(
            f"{item.nodeid}: provider-gated evidence must be explicitly classified as "
            "provider-conformance"
        )
    if explicit:
        marked = LANE_BY_MARKER[explicit.pop()].category
        if marked is not category:
            raise pytest.UsageError(
                f"{item.nodeid}: checked-in marker {marked.value!r} conflicts with "
                f"manifest lane {category.value!r}"
            )
    return category
...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    ...
    for item in items:
        try:
            Path(item.path).resolve().relative_to(Path(str(config.rootpath)).resolve())
        except ValueError as error:
            raise pytest.UsageError(
                f"test item is outside the candidate root: {item.nodeid}"
            ) from error
        category = category_for_item(item, manifest)
        lane = LANE_BY_CATEGORY[category]
        if lane.marker is None:
            raise pytest.UsageError(
                f"{item.nodeid}: accepting evidence lane has no executable marker"
            )
        item.add_marker(lane.marker)
        item.user_properties.append(("arEvidenceCategory", category.value))
        item.user_properties.append(("arEvidenceLaneDigest", manifest.digest))
