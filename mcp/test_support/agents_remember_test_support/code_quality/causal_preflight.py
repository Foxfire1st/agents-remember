...
def evaluate_preflights(
    ...
    *,
    ...
    """Evaluate owners once and block only source-proved exact dependent nodes."""

    outcomes: list[dict[str, object]] = []
    blocked: list[dict[str, object]] = []
    for spec in specs:
        dependencies = derive_causal_nodes(
            project_root,
            owner=spec.owner,
        ...
        try:
            ...
            outcomes.append(_failed_outcome(spec, error, dependencies))
            blocked.extend(_blocked_consumers(spec, dependencies))
        else:
            outcomes.append(_passed_outcome(spec, dependencies))
...
def _passed_outcome(
    spec: PreflightSpec,
    dependencies: tuple[CausalNodeDependency, ...],
) -> dict[str, object]:
    return {
        ...
        "dependentNodes": [item.node_id for item in dependencies],
    }
    ...
    return [
        {
            ...
            "dependencyChain": list(dependency.dependency_chain),
        }
...
def preflight_scope_units() -> str:
    ...
    return (
        f"{len(PREFLIGHTS)} registered owner preflight(s) plus source-derived exact-node "
...
def failed_report(path: Path) -> bool:
    ...
    return is_failed_causal_report(path)
