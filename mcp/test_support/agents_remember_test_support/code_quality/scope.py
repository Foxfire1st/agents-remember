...
class PackageAuthority:
    """Exhaustive ownership of importable packages for behavioral quality rails."""
...
def configured_package_authority(
    project_root: Path,
    tracked: list[Path],
) -> PackageAuthority:
    """Resolve every importable package through explicit operational ownership.

    Importability and installation location are not production authority. The repository must
    classify every top-level package as operational product code or verification infrastructure;
    missing, duplicate, and stale declarations refuse rather than falling back to directory
    placement.
    """

    pyproject, data = read_pyproject(project_root)
    section = toml_section(data, ("tool", "agents_remember"))
    product = _configured_package_roots(section, "product_package_roots", pyproject)
    verification = _configured_package_roots(
        section,
        "verification_package_roots",
        pyproject,
    )
    overlap = set(product) & set(verification)
    if overlap:
        raise ScopeError(
            "package authority assigns roots to both product and verification: "
            + ", ".join(path.as_posix() for path in sorted(overlap))
        )
    detected = set(top_level_packages(tracked))
    declared = set(product) | set(verification)
    missing = sorted(detected - declared, key=Path.as_posix)
    stale = sorted(declared - detected, key=Path.as_posix)
    if missing or stale:
        raise ScopeError(
            "package authority must classify every tracked top-level package exactly once: "
            f"missing={[path.as_posix() for path in missing]}, "
            f"stale={[path.as_posix() for path in stale]}"
        )
    if not product:
        raise ScopeError("product_package_roots must contain at least one operational package")
    return PackageAuthority(product=product, verification=verification)
...
def _configured_package_roots(
    ...
    if not isinstance(raw, list) or not all(isinstance(value, str) for value in raw):
        raise ScopeError(f"[tool.agents_remember] {key} must be a string list in {pyproject}")
    roots: list[Path] = []
    for value in raw:
        normalized = value.strip()
        path = Path(normalized)
        if not normalized or path.is_absolute() or ".." in path.parts or path == Path("."):
            raise ScopeError(
                f"[tool.agents_remember] {key} contains an invalid repository-relative root: "
                f"{value!r}"
            )
        roots.append(path)
    if len(roots) != len(set(roots)):
        raise ScopeError(f"[tool.agents_remember] {key} contains duplicate roots")
    return tuple(roots)
...
def derive_scope(project_root: Path) -> GateScope:
    ...
    Every tracked Python file remains linted, formatted, and type-checked. Coverage and CRAP
    measure only explicitly classified operational product packages. Verification packages remain
    under static and structural rails without recursively acquiring product behavioral proof.
    """
    tracked = git_ls_files(project_root, "*.py")
    if not tracked:
        raise ScopeError(f"git tracks no Python files under {project_root}")
    authority = configured_package_authority(project_root, tracked)
    test_paths = pytest_testpaths(project_root)
    coverage_paths = list(authority.product)
