...
def test_registered_direct_organizational_door_keeps_leaf_review_gate(
    tmp_path: Path,
) -> None:
    """Direct series admission preserves missing, current, blocked, and stale checks."""

    declare_test_process()
    fixture, series = _direct_organizational_fixture(tmp_path)
    source_before = git(series.code_repo_path, "rev-parse", series.code_source_branch)
    evidence = series.task_root / "notes" / "reports" / "direct-review.md"
    evidence.parent.mkdir(parents=True, exist_ok=True)
    evidence.write_text("Direct organizational review evidence.\n", encoding="utf-8")

    missing = _call_direct_door(fixture, series)
    assert missing.isError is True
    missing_text = [block.text for block in missing.content if isinstance(block, TextContent)]
    assert missing_text
    assert "closeout-door-route-review-required" in missing_text[0]
    assert load_contract(series.contract_path).closeout_door is None
    assert git(series.code_repo_path, "rev-parse", series.code_source_branch) == source_before
    ...
    assert begun["reviewState"] == {
        "round": 1,
        "pending": True,
        "baselineFindings": [],
        "remainingFindingIds": [],
        "developerApproval": None,
        "additionalRounds": 0,
    }
    published = _assert_wire_payload(
        _call(
            ...
            "task_doc",
            {
                ...
                "contract_path": series.contract_path.as_posix(),
                "slug": "leaf-a",
                "review": _direct_leaf_review_payload("pass"),
                "branch_addressed": True,
            },
        )
    )
    assert published["ok"] is True
    assert published["reviewState"]["remainingFindingIds"] == []
    declared = _assert_wire_payload(_call_direct_door(fixture, series))
    assert declared["generation"]["reviewProvenance"]["state"] == "proven"
    generation_id = declared["generation"]["generationId"]

    _exercise_blocked_direct_review(tmp_path / "blocked")

    git(series.code_repo_path, "checkout", series.code_work_branch)
    (series.code_repo_path / "direct-change-after-review.txt").write_text(
        "candidate moved after direct review\n", encoding="utf-8"
    )
    git(series.code_repo_path, "add", "direct-change-after-review.txt")
    git(series.code_repo_path, "commit", "-m", "move direct candidate")
    git(series.code_repo_path, "checkout", series.code_source_branch)
    stale = _call_direct_door(
        fixture,
        series,
        expected_generation_id=generation_id,
    )
    assert stale.isError is True
    stale_text = [block.text for block in stale.content if isinstance(block, TextContent)]
    assert stale_text
    assert "closeout-door-route-review-stale" in stale_text[0]
    stale_door = load_contract(series.contract_path).closeout_door
    assert stale_door is not None and stale_door.generationId == generation_id
    assert git(series.code_repo_path, "rev-parse", series.code_source_branch) == source_before
