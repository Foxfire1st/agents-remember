...
``supervisorHeartbeat`` -- were injected into the dumped projection with nothing declaring
...
def _heartbeat() -> SupervisorHeartbeatPayload:
...
class ServedStateTailTests(unittest.TestCase):
    ...
    def test_absent_halves_contribute_no_keys(self) -> None:
        ...
        self.assertEqual(
            set(served_state_tail(build=None, heartbeat=_heartbeat())), {"supervisorHeartbeat"}
        )
    ...
    def test_the_two_halves_serialize_under_opposite_null_rules(self) -> None:
        ...
        # supervisor as an explicit null. One shared ``exclude_none`` dump cannot do both,
        ...
        never_ticked = SupervisorHeartbeatPayload(staleCutoffSeconds=60.0, stale=True)
        ...
        self.assertIsNone(tail["supervisorHeartbeat"]["lastTickAt"])
        self.assertIsNone(tail["supervisorHeartbeat"]["ageSeconds"])
...
class ServedStateRouteConformanceTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # The app's own supervisor loop ticks the heartbeat on startup, which would make the
    ...
    def _tick_supervisor(self, *, age: timedelta) -> None:
        SupervisorHeartbeatStore(observer_root(_config(self.tmp))).tick(now=datetime.now(UTC) - age)
    ...
    def test_state_body_validates_against_the_served_model(self) -> None:
        self._tick_supervisor(age=timedelta(hours=6))
        ...
        assert served.supervisorHeartbeat is not None
        self.assertTrue(served.supervisorHeartbeat.stale)  # six hours past a 60s cutoff
    ...
    def test_a_never_ticked_supervisor_still_serves_a_valid_body(self) -> None:
        # No heartbeat row at all: the nulls are reported, not omitted, and still validate.
        with self._client() as client:
            body = client.get("/api/state").json()
        served = ServedWorkspaceProjection.model_validate(body)
        assert served.supervisorHeartbeat is not None
        self.assertIsNone(served.supervisorHeartbeat.lastTickAt)
        self.assertTrue(served.supervisorHeartbeat.stale)  # never ticked reads as stale
        self.assertIn("lastTickAt", body["supervisorHeartbeat"])  # reported, not dropped
    ...
    def test_the_304_branch_serves_the_etag_and_no_body(self) -> None:
        ...
        with self._client() as client:
            ...
            self._tick_supervisor(age=timedelta(hours=9))  # the volatile half moves...
...
class ServedSnapshotConformanceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_snapshot_validates_and_delta_carries_no_tail(self) -> None:
        ...
        gen = stream_events(projector, build=_build(), supervisor_heartbeat=_heartbeat())
        try:
            ...
            self.assertIsNotNone(served.servingBuild)
            self.assertIsNotNone(served.supervisorHeartbeat)
            self.assertEqual(
    ...
    async def test_a_snapshot_without_a_tail_is_still_a_valid_served_body(self) -> None:
        ...
        self.assertIsNone(served.supervisorHeartbeat)
