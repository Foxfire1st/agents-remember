...
If `--report` is supplied, C-08's resolved `coordination_root` still owns report placement. Relative paths are resolved from the resolved `temp_root`. Absolute paths are only used as-is when they are already inside the resolved coordination root; otherwise the report filename is redirected to `<coordination_root>/temp/drift-reports/<repo-name>/`. The repo/branch-prefixed default filename applies whenever `--report` is omitted.
