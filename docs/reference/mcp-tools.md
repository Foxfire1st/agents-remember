...
| `skills_install` | Copy the packaged (already flat) skills into the harness skill root — one folder per skill `name`; no layout option. | `dry_run=false`, `overwrite=false`, `archive_existing=false` |
