...
describe("DetailPanel series notes (L9)", () => {
  ...
  it("lists the master's notes on a leaf reader and resolves a notes reference to a link", async () => {
    ...
    seedTaskDocuments([doc]);
    const view = render(<DetailPanel selectedId={`taskdoc:${leafPath}`} />);
    ...
    // The notes-file reference is an openable link; the code-path reference stays plain text.
    fireEvent.click(await view.findByTestId("note-ref-1"));
    expect((await view.findByTestId("note-view")).textContent).toContain("the ledger body");
