    ...
    <div className={rowLabelGroup}>
      ...
      {showChip ? (
        <span
          ...
          title={
            visual.key === "awaiting-input" && promptTitled
              ? promptTitled
              : visual.word
          }
...
function railRowDerived(
  ...
  const promptTitled =
    prompt !== undefined && agentLabel !== undefined
      ? `${agentLabel}: ${prompt}`
      : prompt;
...
export function RailRow({
  ...
  const { visual, code, gate, promptTitled, selected, hasEndFailure, showChip } =
    derived;
    ...
    <div className={attnStrip} data-testid="rail-attention-strip">
      {rollup.needsInput.length > 0 ? (
        <button
          ...
          onClick={() =>
            onFocusSet("needsInput", jumpClass({ needsInput: rollup.needsInput }))
          }
      ...
      {rollup.unacked.length > 0 ? (
        <button
          ...
          className={attnButton({ tone: "warn" })}
          onClick={() =>
            onFocusSet("unacked", jumpClass({ unacked: rollup.unacked }))
          }
      ...
      {rollup.working.length > 0 ? (
        <button
          ...
          onClick={() =>
            onFocusSet("working", jumpClass({ working: rollup.working }))
          }
    ...
    >
      {!props.pollHealthy ? (
        <div
          ...
          role="status"
        >
          catalog poll stale — {props.pollMissedBeats} beats missed; rows may
          be frozen
        </div>
      ) : null}
      <AttentionStrip
