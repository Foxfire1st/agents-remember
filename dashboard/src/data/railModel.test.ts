...
describe('buildRailModel (sprint-local command groups and leaf-subordinate clusters)', () => {
  ...
  it("clusters leaf agents per leaf under the manager's master", () => {
    ...
    ); // 01_protocol seats are all landed → completed folder
  });
...
describe('fleet attention (R12)', () => {
  it('rolls up needs-input / failed / working and honors the joins', () => {
    ...
    expect(new Set(rollup.working)).toEqual(
      new Set(['orchestrator', 'manager-l4', 'worker-l4', 'worker-caps']),
    );
  });
  ...
  it('intra-class tiebreak: the longest-waiting seat wins in EVERY class (review finding 4)', () => {
    ...
    expect(
      jumpToAttentionTarget(
        { ...base, unacked: ['reviewer-l4', 'curator-l4'], criticalBus: [] },
        fleet,
      ),
    ).toBe('curator-l4');
    expect(
      jumpToAttentionTarget(
        { ...base, unacked: [], criticalBus: ['reviewer-l4', 'curator-l4'] },
        fleet,
      ),
    ).toBe('curator-l4');
  });
...
describe('projection joins', () => {
  ...
  it('joins HELD gates by leafKey only while undecided (R13)', () => {
    const held = heldGatesByLeafKey(
      [doc('04_serving', 'LC1'), doc('05_capabilities', 'LC2')],
      lifecycles,
    );
  ...
  it('brief column is TWO-state: pending while unacknowledged, gone otherwise — never a tri-state', () => {
    ...
    expect(
      briefPendingSessionIds([pickup({ deliveredToSession: 'worker-l4' })], sessions).has(
        'worker-l4',
      ),
    ).toBe(true);
    expect(
      briefPendingSessionIds([pickup({ lifecycleId: 'LC9' })], sessions).has('worker-l4'),
    ).toBe(true);
    // A non-brief message or an unmatchable pickup never marks the column.
    expect(
      briefPendingSessionIds(
        [pickup({ messageKind: 'message', deliveredToSession: 'worker-l4' })],
        sessions,
      ).size,
    ).toBe(0);
  ...
  it('critical bus = age ≥ ttl·0.8 or escalated check-chat (F11)', () => {
    ...
    expect(
      criticalBusSessionIds([pickup({ lifecycleId: 'LC9', state: 'check-chat' })], sessions).has(
        's',
      ),
    ).toBe(true);
  });
...
describe('question triage (R16)', () => {
  ...
  it('lists ALL waiting seats, newest first', () => {
    ...
    expect(waitingSeats([...fleet, second]).map((seat) => seat.id)).toEqual([
      'waiter-2',
      'worker-tui',
    ]);
  });
